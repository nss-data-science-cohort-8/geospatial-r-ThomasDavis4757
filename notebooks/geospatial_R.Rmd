...
burg_zips <- st_join(burglaries_geo, DC, join = st_within, left = FALSE)
burg_zips
...
DC |> 
  ggplot() +
  geom_sf() +
  geom_sf(data = burg_zips, size = 0.1, aes(color = `NAMELSAD`)) +
...
census <- census |> 
  mutate(
    NAMELSAD = str_extract(`NAME`, "^Census Tract [0-9]+(?:\\.[0-9]+)?")
  )

census

total <- left_join(census, burg_zips, by = 'NAMELSAD')
...
total_burgs
```
The census with the highest number of burglaries was Tract 160.


```{r}
total_burgs_1000 <- total |> 
  distinct(incident_number, .keep_all = TRUE) |> 
  group_by(`NAMELSAD`, `population`) |> 
  summarise(n = n()) |> 
...
```
The census with highest burglaries per 1000 was tract 160

```{r}
total_burgs_1000_med <- total |> 
  
  distinct(incident_number, .keep_all = TRUE) |> 
  group_by(`NAMELSAD`, `population`, `median_income`) |> 
  summarise(n = n()) |> 
  mutate(per_1000 = (n / population) * 1000) |> 
  arrange(desc(per_1000)) 
total_burgs_1000_med
  
```


```{r}
ggplot(total_burgs_1000_med, aes(x = per_1000, y = median_income)) + geom_point() + ylim(0,200000)
```

```{r}
ggplot(total_burgs_1000_med, aes(x = n, y = median_income)) + geom_point() + ylim(0,200000)
```
